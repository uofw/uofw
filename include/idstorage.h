...
#define SCE_ID_STORAGE_LEAF_IDPS_OPEN_PSID_1                    0x100
#define SCE_ID_STORAGE_LEAF_IDPS_OPEN_PSID_2                    0x101
#define SCE_ID_STORAGE_LEAF_IDPS_OPEN_PSID_3_UMD_1              0x102
...
#define SCE_ID_STORAGE_LEAF_ID_BACKUP_IDPS_OPEN_PSID_1          0x120
#define SCE_ID_STORAGE_LEAF_ID_BACKUP_IDPS_OPEN_PSID_2          0x121
#define SCE_ID_STORAGE_LEAF_ID_BACKUP_IDPS_OPEN_PSID_3_UMD_1    0x122
...
/* Offset into the leaf [SCE_ID_STORAGE_LEAF_IDPS_OPEN_PSID_1] to the start of the first IDPS certificate. */
#define SCE_ID_STORAGE_LEAF_IDPS_OPEN_PSID_1_OFFSET_IDPS_CERTIFICATE_1       0x38
/* Offset into the leaf [SCE_ID_STORAGE_LEAF_IDPS_OPEN_PSID_2] to the start of the OpenPSID certificate. */
#define SCE_ID_STORAGE_LEAF_IDPS_OPEN_PSID_2_OFFSET_OPEN_PSID_CERTIFICATE    0x1D0

#define SCE_ID_STORAGE_LEAF_IDPS_OPEN_PSID_3_UMD_1_OFFSET_REGION_CODES       0xB0
 ...
 *                                .
 * 
 * The leafs 0x100 - 0x102 contain the IDPS (termed "PSID" on the PSP) and the OpenPSID. As an additional security feature
 * to prevent successfully tampering them, they are stored inside a certificate, which can be used to verify the IDPS and
 * OpenPSID. For more info, see SceIdStorageIDPSCertificate and SceIdStorageOpenPSIDCertificate.
 * 
 * These leafs are "continious leafs" and contain 5 IDPS certificates (totalling 5 * 184 = 920 bytes)
 * followed by a single OpenPSID certificate (for a total size of 920 + 184 = 1104 bytes). 
 * 
 * Of these 5 certificates, only the first IDPS certificate and the OpenPSID certificate are used in the system.
 * It is unknown what the purpose the remaining four IDPS certificates is.
 * 
 * +------------------------------------------------------------+----------------
 * |                 unknown data (size: 0x38)                  |                |
 * |------------------------------------------------------------|                |
 * |          SceIdStorageIDPSCertificate (size: 0xB8)          |                |
 * |------------------------------------------------------------|            Leaf 0x100
 * |          SceIdStorageIDPSCertificate (size: 0xB8)          |                |
 * |------------------------------------------------------------|                |
 * |       SceIdStorageIDPSCertificate_part1 (size: 0x58)       |                |
 * |------------------------------------------------------------|----------------
 * |       SceIdStorageIDPSCertificate_part2 (size: 0x60)       |                |
 * |------------------------------------------------------------|                |
 * |          SceIdStorageIDPSCertificate (size: 0xB8)          |                |
 * |------------------------------------------------------------|            Leaf 0x101
 * |          SceIdStorageIDPSCertificate (size: 0xB8)          |                |
...
/*
 * This structure contains the IDPS ("PSID" on the PSP) and an ECDSA signature used to verify the correctness of the 
 * IDPS.
 * The IDPS is used, for example, in PSN DRM, DNAS and system configuration (with its derived PSCode).
 */
typedef struct {
	...
	u8 hash[0x10]; // 168
} SceIdStorageIDPSCertificate; // size = 184
